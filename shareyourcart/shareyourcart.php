...
function shareyourcart_options()
{
    ...
    {       
        ...
        elseif($_POST['account-form'] == 'account-form')
        {
            ...
            if($wpdb->update($wpdb->base_prefix.'shareyourcart_settings',$update_row,Array('id'=>1)))
                $status_message = '<div class="updated settings-error"><p><strong>Account settings successfully saved.</strong></p></div>';
            else
                $status_message = '<div class="updated settings-error"><p><strong>Account settings are not saved.</strong></p></div>';
        }
...
{        
    ...
    {                     
        ...
        $settings = $wpdb->get_results("SELECT app_key, client_id FROM ".$wpdb->base_prefix."shareyourcart_settings WHERE id=1");    
